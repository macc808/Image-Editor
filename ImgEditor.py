...
from PyQt5.QtCore import Qt,QRect # потрібна константа Qt.KeepAspectRatio для зміни розмірів із збереженням пропорцій
from PyQt5.QtGui import QPixmap # оптимізована для показу на екрані картинка
 
from PIL import Image
from PIL.ImageQt import * # Для перенесення графіки з Pillow до QT
...
class ImageProcessor():
   ...
   def clearss(self):
    # Перезавантаження оригінального зображення
    if self.filename:
        original_path = os.path.join(workdir, self.filename)
        self.image = Image.open(original_path)
        self.showImage(original_path)  # Показуємо оригінальне зображення
    else:
...
btn_clear.clicked.connect(workimage.clearss)
app.exec()
